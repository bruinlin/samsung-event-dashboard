...
Documents & Deliverables sits directly below Attention Needed in the side column. Events with `resourceLinks` display those external cards alongside—not instead of—private PDF Documents. Safe document metadata may be visible publicly, but Preview and Download actions require an authenticated Viewer, Editor or Admin with event access and obtain a short-lived private Storage URL. Browser PDF upload is deliberately not available. No Storage object path is exposed to public viewers.
...
- `data/OCTS_2026.js` contains `finalDocuments`: public-safe metadata for private Preview or Final PDFs. Keep private object paths and signed URLs out of all `data/*.js` files. For a new private PDF, add its metadata card first, then run the local administrator uploader.
...
2. Add the safe card to `data/<EVENT_ID>.js` first: unique `id`, optional `logicalDocumentId` for version grouping, names, category, subcategory, version, lifecycle, date, format, size and `downloadable: true`. Do not add a local path, object path or URL.
3. The repository owner runs `node scripts/upload-private-document.mjs` locally. It verifies that the prompted metadata exactly matches the safe card, uploads a unique object to private Storage, writes the legacy `document_files` mapping and verifies a five-minute signed URL. It never overwrites a prior version.
4. Viewer, Editor and Admin can Preview and Download assigned-event files. Public and unapproved viewers can see only safe metadata and cannot obtain a signed URL. No Dashboard role can upload PDFs in the browser.
5. Inspect any source for sensitive content, comments/notes, contacts, amounts, local paths, private links and embedded content before uploading.
